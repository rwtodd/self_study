...
(defun ulam-next (sofar backlog)
  (let* ((batch (ulam-backl sofar))
         ...
         (nval   (pop result)))
    (cons nval (lambda () (ulam-next (cons nval sofar) result)))))

(defun ulam (a b) 
  (cons a (lambda () (ulam-next (list a) (list (ulam-ent b nil))))))

; LCDR and LTAKE are a couple helper functions since I'm not using
; a lazy-list library... 
